# p4.py  – heuristic vs LP comparison with command-line summary
import numpy as np
import pandas as pd
import time
import importlib
...
# ---------------------------------------------------------------------------
# bring in LP relaxation solver from p3.py (must define solve_lp_relaxation)
# ---------------------------------------------------------------------------
try:
    p3 = importlib.import_module("p3_modified")
...
for s_id, (sc_idx, cc_idx, hc_idx) in enumerate(SCENARIOS, start=1):
    for inst_id in range(1, INSTANCES_PER_SCENARIO + 1):
        ...
        (N, T, D, I0, I, CH, CP, CV1, CV2, CV3,
         CF_vals, Vi, cont_cost) = generate_instance(sc_idx, cc_idx, hc_idx, rng)
